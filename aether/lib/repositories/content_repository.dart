...
class ContentRepository {
  ...
  /// Get a content item by ID (alias for getContentItem)
  ...
  ContentItem _updateParentFolderId(
    ContentItem item,
  ...
  Future<List<ContentItem>> _getAllChildrenInTrash(String folderId) async {
    ...
    for (final map in maps) {
      final item = await _storageService.getContentItemById(
        map['id'] as String,
      );
      if (item != null) {
        items.add(item);
  ...
  Future<List<ContentItem>> searchItems(
    String query, {
    String? folderId,
  }) async {
